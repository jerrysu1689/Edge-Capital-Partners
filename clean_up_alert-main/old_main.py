    ...
    for row in raw_rows:
        ...
        if action and price:
            ...
            trade = {
                ...
                'Actual Shares': shares  # Store for reference only
            }
...
def match_buys_and_sells_state_based(trades):
    """
    State-based matching: Simple 1-to-1 matching
    - All shares treated as 1 (no weighted pricing)
    - Match on Alert ID + Ticker + Timestamp order
    - Handles shorts (sell before buy) and longs (buy before sell)
    """
    closed_trades = []
    open_positions = []
    ...
    for (alert_id, ticker), group_trades in trade_groups.items():
        # Sort by datetime (includes hour, minute, second precision)
        group_trades.sort(key=lambda x: x.get('DateTime', datetime.min))

        # State machine: position can be 0 (flat), +1 (long), or -1 (short)
        position = 0
        entry_trade = None
        ...
        for trade in group_trades:
            ...
            # All trades are 1 share - no weighted pricing needed
            shares = 1
            ...
            if action == 'Buy':
                if position == 0:
                    ...
                    # Closing short position (sell → buy)
                    ...
                    # Short P&L: profit when sell high, buy low
                    ...
                    result = {
                        ...
                        'Status': 'Closed',
                        'Actual Buy Shares': trade.get('Actual Shares', ''),
                        'Actual Sell Shares': entry_trade.get('Actual Shares', '')
                    }
                    ...
                    # Already long, consecutive buy - ignore (pyramiding not supported in 1-share model)
                    pass
                ...
                if position == 0:
                    ...
                    # Closing long position (buy → sell)
                    ...
                    # Long P&L: profit when buy low, sell high
                    ...
                    result = {
                        ...
                        'Outcome': 'Win' if ret_pct > 0 else 'Loss',
                        'Status': 'Closed',
                        'Actual Buy Shares': entry_trade.get('Actual Shares', ''),
                        'Actual Sell Shares': trade.get('Actual Shares', '')
                    }
                    ...
                    # Already short, consecutive sell - ignore
                    pass
        ...
        if position != 0 and entry_trade is not None:
            open_position = {
                ...
                'Actual Shares': entry_trade.get('Actual Shares', '')
            }
...
if __name__ == "__main__":
    ...
    input_file = pr_root / 'clean_up_alerts' / 'inputs' / 'consolidated_alerts_LTD_20250819.csv'
